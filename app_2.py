...
# ─────────────────────────────────────────────
# DATABASE SETUP (SQLite - persists data)
# ─────────────────────────────────────────────
...
def init_db():
    ...
    conn.executescript('''
        ...
        CREATE TABLE IF NOT EXISTS projects (
            ...
            freelancer_accepted INTEGER DEFAULT 0,
            deadline TEXT,
...
def log_activity(text, project_id=None, user_id=None):
    ...
    db.execute('INSERT INTO activity VALUES (?,?,?,?,?)',
               (gen_id(), now(), text, project_id, user_id))
    db.commit()
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .milestone-item{display:flex;align-items:center;gap:12px;padding:14px 18px;border:1px solid var(--border);margin-bottom:2px;background:var(--navy3);transition:border-color 0.2s;flex-wrap:wrap;}
  ...
  .modal{background:var(--navy2);border:1px solid var(--border);padding:32px;max-width:480px;width:90%;position:relative;}
  ...
  <div class="container">
    <div style="padding:40px 0 80px;max-width:680px;">
      ...
      <div class="card">
        ...
        <div class="form-group"><label>Description & Deliverables</label><textarea id="projDesc" rows="4" placeholder="Describe what needs to be done, deliverables, requirements..."></textarea></div>
        <div class="form-row">
      ...
      <div id="balanceWarning" style="display:none;" class="alert alert-err">Insufficient balance. <a onclick="openModal()" style="color:var(--amber);cursor:pointer;">Add funds →</a></div>
...
async function renderDashboard(){
  ...
  actions.innerHTML=currentUser.role==='client'
    ?`<button class="btn btn-outline btn-sm" onclick="openModal()">💰 Add Funds</button><button class="btn btn-amber" onclick="showPage('newproject')">+ New Project</button>`
    :'';
...
async function renderProjectsList(el){
  ...
  if(!projects.length){
    el.innerHTML=`<div class="empty-state"><div class="icon">📋</div><div>${currentUser.role==='client'?'No projects yet. Create your first one.':'No projects assigned to you yet.'}</div></div>`;
    return;
  }
...
async function renderProfile(el){
  ...
  el.innerHTML=`<div class="card" style="max-width:520px;">
    ...
    ${currentUser.role==='client'?'<button class="btn btn-amber" onclick="openModal()" style="margin-top:12px;">💰 Add Funds</button>':''}
  </div>`;
}
...
async function viewProject(id){
  ...
  const milestonesHTML=p.milestones.map((m,i)=>{
    let actions='';
    if(isFreelancer&&m.status==='pending'&&p.freelancer_accepted) actions=`<button class="btn btn-amber btn-sm" onclick="submitMilestone('${p.id}','${m.id}')">Submit Work</button>`;
  ...
  const acceptBtn=isFreelancer&&p.status==='pending'?`<button class="btn btn-amber" onclick="acceptProject('${p.id}')">✅ Accept & Sign Contract</button>`:'';
  const cancelBtn=isClient&&p.status==='pending'?`<button class="btn btn-danger btn-sm" onclick="cancelProject('${p.id}')">Cancel Project</button>`:'';
    ...
    <div style="display:grid;grid-template-columns:1fr 320px;gap:20px;align-items:start;">
      <div>
        ...
        <div class="card">
          ...
          <div class="text-muted mt-8">${completedCount(p)} of ${p.milestones.length} milestones complete · $${Number(p.released).toFixed(2)} released</div>
        </div>
        <div class="card">
...
async function cancelProject(id){if(!confirm('Cancel this project? Remaining escrowed funds will be returned.'))return;const r=await api('/api/projects/'+id+'/cancel','POST');if(r.ok)showPage('dashboard');}
...
function openModal(){document.getElementById('topupModal').classList.add('active');}
...
async function topUp(amount){
  ...
  if(r.ok){
    ...
    setTimeout(()=>{closeModal();if(currentTab==='profile')switchTab('profile');},1500);
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            if u['role'] == 'client':
                ...
                rows = db.execute('SELECT * FROM projects WHERE freelancer_email=? ORDER BY created_at DESC', (u['email'],)).fetchall()
            projects = []
            for row in rows:
                p = row_to_dict(row)
            ...
            if proj_filter:
                ...
                rows = db.execute('''SELECT a.* FROM activity a
                    LEFT JOIN projects p ON a.project_id=p.id
                    WHERE p.client_id=? OR a.project_id IS NULL
                    ORDER BY a.time DESC LIMIT 30''', (u['id'],)).fetchall()
            else:
                rows = db.execute('''SELECT a.* FROM activity a
                    LEFT JOIN projects p ON a.project_id=p.id
                    WHERE p.freelancer_email=? OR a.project_id IS NULL
                    ORDER BY a.time DESC LIMIT 30''', (u['email'],)).fetchall()
    ...
    def do_POST(self):
        ...
        if path == '/api/register':
            ...
            db = get_db()
            existing = db.execute('SELECT id FROM users WHERE email=?', (d['email'].lower().strip(),)).fetchone()
            if existing: self.send_json({'ok': False, 'error': 'Email already registered'}); return
            uid = gen_id()
            db.execute('INSERT INTO users VALUES (?,?,?,?,?,?,?)',
                       (uid, d['name'].strip(), d['email'].lower().strip(),
                        hash_pass(d['password']), d.get('role','client'), 10000.0, now()))
            db.commit()
            sid = self.set_session(uid)
            log_activity(f"New user registered: {d['name']} ({d.get('role','client')})")
            safe = {'id': uid, 'name': d['name'].strip(), 'email': d['email'].lower().strip(),
                    'role': d.get('role','client'), 'balance': 10000.0}
            self.send_json({'ok': True, 'user': safe}, cookie=f'session={sid}; Path=/; SameSite=Lax')

        elif path == '/api/login':
            db = get_db()
            row = db.execute('SELECT * FROM users WHERE email=?', (d.get('email','').lower().strip(),)).fetchone()
            ...
            pid = gen_id()
            freelancer = row_to_dict(db.execute('SELECT * FROM users WHERE email=?', (d['freelancer_email'].lower().strip(),)).fetchone())
            db.execute('INSERT INTO projects VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)', (
                pid, d['title'], d.get('description',''),
                u['id'], u['name'],
                d['freelancer_email'].lower().strip(),
                freelancer['id'] if freelancer else None,
                freelancer['name'] if freelancer else None,
            ...
            db.commit()
            log_activity(f"{u['name'] if u else 'Client'} cancelled project '{p['title']}' — ${remaining:.2f} returned", pid, u['id'] if u else None)
